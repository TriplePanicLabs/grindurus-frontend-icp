...
function PoolsTable() {
  ...
  const formatTableData = (poolNFTInfos: IPoolsNFTLens.PoolNFTInfoStructOutput[]): PoolData[] => {
    return poolNFTInfos.map(info => {
      ...
      const start =
        timestamp !== 0n ? new Date(Number(info.startTimestamp) * 1000).toLocaleDateString() : 'N/A'
      ...
      const apr =
        aprDenominator > 0 ? `${((aprNumerator / aprDenominator) * 100).toFixed(2)}%` : 'N/A'
    ...
    <>
      <div className={`${styles['header']} table-header`}>
        ...
        <div className={`${styles['search']} table-search`}>
          <input onChange={handleSearch} placeholder="Search with pool id" type="text" />
        </div>
      </div>
        ...
        <div className={styles['pool']} key={index}>
          <div className={styles['content']}>
            <div className={styles['pool-header']}>
              ...
              <div className={styles['pool-header-right']}>{`${Number(data.quoteAmount).toFixed(
                6,
              )} ${data.quoteTokenSymbol} / ${Number(data.baseAmount).toFixed(6)} ${
          ...
          <div className={styles['buttons']}>
            ...
            >
              Buy Royalty
              <div>
                {data.royaltyPrice} {data.quoteTokenSymbol}
              </div>
            </button>
